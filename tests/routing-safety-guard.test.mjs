...
const HOOK_PATH = join(PROJECT_ROOT, '.claude', 'hooks', 'routing-safety-guard.mjs');
...
describe('routing-safety-guard', () => {
  it('allows tools when routing has not started', async () => {
  ...
  it('blocks Grep during routing in progress', async () => {
    ...
    const res = await runHook({
      tool_name: 'Grep',
      tool_input: { pattern: 'router', path: '.' },
      context: {},
    });
    assert.equal(res.json.decision, 'block');
    assert.match(res.json.reason, /ROUTING SAFETY GUARD/);
  });

  it('blocks Search during routing in progress', async () => {
    await mkdir(join(HOOK_TMP_DIR, 'routing-sessions'), { recursive: true });
    await writeFile(
      statePath(),
      JSON.stringify(
    ...
    const res = await runHook({
      tool_name: 'Search',
      tool_input: { query: 'router', path: '.' },
      context: {},
    });
    assert.equal(res.json.decision, 'block');
  });

  it('allows small config globs during routing', async () => {
    await mkdir(join(HOOK_TMP_DIR, 'routing-sessions'), { recursive: true });
    await writeFile(
      statePath(),
      JSON.stringify(
  ...
  it('blocks .claude/context globs during routing', async () => {
    await mkdir(join(HOOK_TMP_DIR, 'routing-sessions'), { recursive: true });
    await writeFile(
      statePath(),
      JSON.stringify(
    ...
    const res = await runHook({
      tool_name: 'Glob',
      tool_input: { glob: '.claude/context/**' },
      context: { agent_name: 'router' },
    });
    assert.equal(res.json.decision, 'block');
  });

  it('blocks broad .claude/** globs during routing', async () => {
    await mkdir(join(HOOK_TMP_DIR, 'routing-sessions'), { recursive: true });
    await writeFile(
      statePath(),
      JSON.stringify(
    ...
    const res = await runHook({
      tool_name: 'Glob',
      tool_input: { glob: '.claude/**' },
      context: { agent_name: 'router' },
    });
    assert.equal(res.json.decision, 'block');
  });

  it('allows tools once routing is completed', async () => {
    await mkdir(join(HOOK_TMP_DIR, 'routing-sessions'), { recursive: true });
    await writeFile(
      statePath(),
      JSON.stringify(
    ...
    const grep = await runHook({
      tool_name: 'Grep',
      tool_input: { pattern: 'x', path: '.' },
      context: {},
    });
    assert.equal(grep.json.decision, 'approve');

    const glob = await runHook({ tool_name: 'Glob', tool_input: { glob: '**/*' }, context: {} });
    assert.equal(glob.json.decision, 'approve');
  });
});
