...
const runCujTestContent = `#!/usr/bin/env node
/**
...
describe('run-cuj.mjs CLI Tests', () => {
  ...
  it('should validate CUJ-001', async () => {
    const result = await runCuj(['--validate', 'CUJ-001'], { timeout: 15000 });
    assert.strictEqual(result.code, 0, 'Validation should pass');
  });
...
fs.writeFileSync('tests/run-cuj.test.mjs', runCujTestContent);
console.log('Created tests/run-cuj.test.mjs');
