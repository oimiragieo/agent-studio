...
The rule index version validation system ensures that the `.claude/context/config/rule-index.json` file is up-to-date with the expected version. This prevents Skills from consuming outdated index structures that may be missing new metadata or have incompatible formats.
...
1. **Read Index**: Load `.claude/context/config/rule-index.json`
