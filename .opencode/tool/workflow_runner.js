...
function findStepInWorkflow(yamlContent, stepNumber) {
  ...
  const inputs = inputsMatch ? inputsMatch[1].split('\n').filter(l => l.trim().startsWith('-')).map(l => l.trim().substring(2).trim()) : [];
  
  // Extract outputs
  const outputsMatch = block.match(/outputs:\s*\n((?:\s+-\s+.*\n)+)/);
  const outputs = outputsMatch ? outputsMatch[1].split('\n').filter(l => l.trim().startsWith('-')).map(l => l.trim().substring(2).trim()) : [];
 ...
 */
function validateStepDependencies(yamlContent, stepNumber, workflowId, storyId = null, epicId = null) {
  ...
  if (step.inputs && Array.isArray(step.inputs)) {
    for (const input of step.inputs) {
      ...
      if (ref && ref.fromStep) {
        ...
        if (sourceStep.outputs && Array.isArray(sourceStep.outputs)) {
          for (const output of sourceStep.outputs) {
            ...
            if (outputName) {
              ...
              let interpolatedOutput = interpolateTemplateVariables(outputName, workflowId, storyId, epicId);
              let interpolatedArtifact = interpolateTemplateVariables(ref.artifact, workflowId, storyId, epicId);
        ...
        if (!outputFound) {
          errors.push(`Input '${input}' references artifact '${ref.artifact}' from step ${ref.fromStep}, but that step does not produce this artifact`);
          continue;
        }
        ...
        if (!existsSync(artifactPath)) {
          if (ref.optional) {
            console.warn(`⚠️  Warning: Optional artifact '${interpolatedArtifact}' from step ${ref.fromStep} not found.`);
          } else {
            missing.push({
...
function validateWorkflowInputs(yamlContent, providedInputs = {}) {
  const workflowInputsMatch = yamlContent.match(/workflow_inputs:\s*\n\s+required:\s*\n((?:\s+-\s+.*\n)+)/);
...
function main() {
  ...
  if (!args.workflow || !args.step) {
    ...
    console.error('Usage: node workflow_runner.js --workflow <yaml-path> --step <number> [--id <workflow-id>] [--story-id <story-id>] [--epic-id <epic-id>]');
    process.exit(1);
  }
  ...
  if (args.step === '0' || args.step === 0) {
    ...
    if (!inputValidation.valid) {
      ...
      console.error('Example: --inputs \'{"target_files": ["src/"], "coding_standards": "PEP 8"}\'');
      process.exit(1);
    }
  ...
  if (!existsSync(inputPath)) {
    ...
    console.error(`   Please ensure the agent has created '${interpolatedOutput}' before running validation.`);
    process.exit(1);
  }
  ...
  try {
    ...
    execSync(`node "${gateScript}" ${schemaArg} --input "${inputPath}" --gate "${gatePath}" --autofix 1`, { stdio: 'inherit' });
  ...
  if (config.secondaryOutputs && config.secondaryOutputs.length > 0) {
    ...
    for (const secondary of config.secondaryOutputs) {
      const interpolatedArtifact = interpolateTemplateVariables(secondary.artifact, workflowId, storyId, epicId);
      ...
      if (secondary.schema) {
        ...
        if (existsSync(secondarySchemaPath)) {
          try {
            ...
            execSync(`node "${gateScript}" ${secondarySchemaArg} --input "${secondaryPath}" --gate "${secondaryGatePath}" --autofix 1`, { stdio: 'inherit' });
          ...
          console.warn(`⚠️  Warning: Schema file not found for secondary output: ${secondary.schema}`);
        }
      } else {
        console.log(`⚠️  Warning: No schema specified for secondary output: ${interpolatedArtifact}`);
      }
    }
  }
