...
function testModuleExports() {
  ...
  assert(typeof hook.isWorkflowComplete === 'function', 'Should export isWorkflowComplete function');
...
function testWorkflowCompletionDetection() {
  ...
  assert(
    !hook.isWorkflowComplete('Starting the workflow now'),
    'Should not detect workflow start'
  );
  assert(
    !hook.isWorkflowComplete('Just fixing a bug'),
...
function testValidateTaskWithCompletion() {
  ...
  assert(
    ...
    'Should append learnings to file'
  );
  assert(
    afterContent.includes('Auto-Extracted'),
    'Should include auto-extracted marker'
  );
  assert(
    afterContent.includes('Test Workflow Run'),
