...
async function main() {
    ...
    try {
        ...
        const tokenBudget = manager.calculateTokenBudget(
            context.conversationTokens,
            context.maxTokens
        );
        ...
        const injection = await Promise.race([
            ...
            new Promise((_, reject) =>
                setTimeout(() => reject(new Error('Injection timeout')), 100)
            )
        ]);
        ...
        // Log performance
        console.error(`[Memory Injection] Tool: ${toolName}, Tokens: ${injection.tokensUsed}/${tokenBudget}, Duration: ${duration}ms`);
