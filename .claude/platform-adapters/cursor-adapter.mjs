...
export class CursorAdapter extends BasePlatformAdapter {
  constructor() {
    ...
    this.skillPromptMap = {
      'response-rater':
        'Rate this plan on a scale of 1-10 based on completeness, feasibility, risk mitigation, agent coverage, and integration. Provide specific feedback for improvement.',
      'artifact-publisher':
        'Publish the generated artifacts to the project feed with proper versioning.',
      ...
      'sequential-thinking':
        'Break down this complex problem step-by-step with explicit reasoning.',
    };

    this.agentPromptMap = {
      architect:
        'You are a system architect. Design systems with scalability, security, and maintainability as primary concerns. Use industry best practices.',
      developer:
        'You are a software developer. Write clean, tested, production-ready code following project conventions and best practices.',
      qa: 'You are a QA engineer. Ensure code quality through comprehensive testing, edge case analysis, and validation.',
      'security-architect':
        'You are a security architect. Evaluate security implications, identify vulnerabilities, and recommend secure implementations.',
      'code-reviewer':
        'You are a code reviewer. Analyze code quality, identify issues, suggest improvements, and ensure adherence to standards.',
      planner:
        'You are a project planner. Create comprehensive plans with clear steps, dependencies, risks, and success criteria.',
      analyst:
        'You are a technical analyst. Research requirements, analyze patterns, and provide evidence-based recommendations.',
      pm: 'You are a product manager. Define requirements, prioritize features, and ensure alignment with business objectives.',
      'ux-expert':
        'You are a UX expert. Design intuitive interfaces following accessibility and usability best practices.',
      devops:
        'You are a DevOps engineer. Design infrastructure, CI/CD pipelines, and deployment strategies.',
      'technical-writer':
        'You are a technical writer. Create clear, comprehensive documentation for developers and users.',
    };
  }
  ...
  translateStep(step) {
    ...
    if (step.outputs) {
      translated.expected_files = step.outputs.map(o =>
        o.replace(/\{\{workflow_id\}\}/g, '${workflow_id}').replace(/\{\{run_id\}\}/g, '${run_id}')
      );
    }
