...
export class FactoryAdapter extends BasePlatformAdapter {
  constructor() {
    ...
    this.agentPromptMap = {
      'architect': 'You are a system architect. Design systems with scalability and maintainability in mind. Follow industry best practices for system design.',
      'developer': 'You are a software developer. Write clean, tested, production-ready code. Follow coding standards and best practices.',
      ...
      'security-architect': 'You are a security architect. Evaluate and design secure systems. Identify vulnerabilities and recommend mitigations.',
      'code-reviewer': 'You are a code reviewer. Analyze code quality, identify issues, and suggest improvements.',
      'planner': 'You are a project planner. Create detailed plans with clear steps, dependencies, and success criteria.',
      'analyst': 'You are a technical analyst. Research and analyze requirements. Provide data-driven recommendations.',
      'pm': 'You are a product manager. Define requirements and prioritize features based on business value.',
      'ux-expert': 'You are a UX expert. Design intuitive, accessible interfaces following UX best practices.',
   ...
   */
  agentToPrompt(agent) {
    return this.agentPromptMap[agent] || `You are a ${agent}. Perform tasks related to ${agent} responsibilities.`;
  }
  ...
  estimateManualEffort(workflow) {
    ...
    return {
      ...
      recommendation: steps.length > 5 ? 'Consider using Claude Code for complex workflows' : 'Feasible with manual coordination'
    };
  }
