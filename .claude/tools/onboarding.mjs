...
const rl = interactive ? createInterface({
  input: process.stdin,
...
const checklist = {
  ...
  'New Features': [
    ...
    { name: 'claude-md-generator skill', path: '.claude/skills/claude-md-generator/SKILL.md', required: true },
    { name: 'plan-generator skill', path: '.claude/skills/plan-generator/SKILL.md', required: true },
    { name: 'diagram-generator skill', path: '.claude/skills/diagram-generator/SKILL.md', required: true },
    { name: 'test-generator skill', path: '.claude/skills/test-generator/SKILL.md', required: true },
    { name: 'dependency-analyzer skill', path: '.claude/skills/dependency-analyzer/SKILL.md', required: true },
...
function checkItem(item) {
  ...
  if (exists && item.path.includes('/') && !item.path.endsWith('.md') && !item.path.endsWith('.yaml')) {
    try {
...
function printResults(categoryResults) {
  ...
  for (const catResult of categoryResults) {
    ...
    console.log(`  ${catResult.requiredPassed}/${catResult.required} required, ${catResult.passed}/${catResult.total} total`);
