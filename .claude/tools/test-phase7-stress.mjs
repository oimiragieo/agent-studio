...
function logMetrics(label = '') {
  ...
  console.log(
    '  Heap: ' +
      usage.heapUsedMB.toFixed(2) +
      'MB / ' +
      CONFIG.maxHeapMB +
      'MB (' +
      ((usage.heapUsedMB / CONFIG.maxHeapMB) * 100).toFixed(1) +
      '%)'
  );
  console.log(
    '  RSS: ' +
      usage.rssMB.toFixed(2) +
      'MB / ' +
      CONFIG.maxHeapMB +
      'MB (' +
      ((usage.rssMB / CONFIG.maxHeapMB) * 100).toFixed(1) +
      '%)'
  ); // NEW
  console.log(
    '  Peak: Heap ' +
      metrics.heapPeak.toFixed(2) +
      'MB, RSS ' +
      metrics.rssPeak.toFixed(2) +
      'MB, Spawns: ' +
      metrics.spawnCount +
      ', Cleanups: ' +
...
async function runBackgroundTask(taskId) {
  ...
  while (!isShuttingDown) {
    ...
    if (isShuttingDown) break;
    console.log(
      '[Background ' + taskId + '] Tick - Heap: ' + getMemoryUsage().heapUsedMB.toFixed(2) + 'MB'
    );
  }
}
...
function handleMemoryPressure(level, usage, stats) {
  ...
  console.warn(
    '[PRESSURE] ' + level.toUpperCase() + ' - Heap: ' + stats.heapUsedMB.toFixed(2) + 'MB'
  );
  if (level === 'critical') {
...
async function shutdown() {
  ...
  if (stopPressureMonitor) {
    stopPressureMonitor();
    stopPressureMonitor = null;
  }
...
async function generateReport() {
  ...
  const report = {
    ...
    results: {
      ...
      completion_status: metrics.exitReason || 'success',
  ...
  if (metrics.exitCode === 1) {
    ...
    report.justification =
      'Graceful degradation triggered before crash - checkpoint saved for restart';
  } else if (metrics.rssPeak >= passThreshold) {
    report.pass_fail_decision = 'CONCERNS';
    report.justification =
      'Peak RSS reached ' +
      ((metrics.rssPeak / CONFIG.maxHeapMB) * 100).toFixed(1) +
      '% (' +
      metrics.rssPeak.toFixed(2) +
      'MB) - exceeds 85% threshold but completed';
  } else {
    report.pass_fail_decision = 'PASS';
    report.justification =
      'Stress test completed successfully. Peak RSS: ' +
      ((metrics.rssPeak / CONFIG.maxHeapMB) * 100).toFixed(1) +
      '% (' +
      metrics.rssPeak.toFixed(2) +
      'MB)';
  ...
  if (metrics.pressureEvents.critical > 0)
    report.recommendations.push(
      'Critical pressure events occurred - consider lower spawn concurrency'
    );
  if (metrics.heapPeak > CONFIG.warningThresholdMB)
    report.recommendations.push('Peak exceeded warning threshold - monitor production workloads');
  if (metrics.cleanupCount > 10)
    report.recommendations.push(
      'Frequent cleanups indicate memory pressure - optimize artifact handling'
    );
...
async function runStressTest() {
  ...
  fs.writeFileSync(
    path.join(reportsDir, 'phase7-stress-test-results.json'),
  ...
  const markdown =
    ...
    '| Metric | Value |\n|--------|-------|\n' +
    '| Duration | ' +
    report.results.duration_minutes +
    ' minutes |\n' +
    '| Status | ' +
    report.results.completion_status +
    ' |\n' +
    '| Exit Code | ' +
    ...
    '| Metric | Value |\n|--------|-------|\n' +
    '| Heap Start | ' +
    report.memory_metrics.heap_start_mb +
    ' MB |\n' +
    '| Heap Peak | ' +
    report.memory_metrics.heap_peak_mb +
    ' MB |\n' +
    '| Heap End | ' +
    report.memory_metrics.heap_end_mb +
    ' MB |\n' +
    '| Heap Limit | ' +
    report.memory_metrics.heap_limit_mb +
    ' MB |\n' +
    '| Peak Usage | ' +
    report.memory_metrics.peak_usage_percent +
    '% |\n\n' +
    '## Event Counts\n' +
    '| Event | Count |\n|-------|-------|\n' +
    '| GC Invocations | ' +
    report.event_counts.gc_count +
    ' |\n' +
    '| Cache Cleanups | ' +
    report.event_counts.cleanup_count +
    ' |\n' +
    '| Subagent Spawns | ' +
    report.event_counts.spawn_count +
    ' |\n' +
    '| Concurrent Peak | ' +
    report.event_counts.concurrent_peak_spawns +
    ' |\n' +
    '| Pressure (High) | ' +
    report.event_counts.pressure_events_high +
    ' |\n' +
    '| Pressure (Critical) | ' +
    report.event_counts.pressure_events_critical +
    ' |\n' +
    '| Checkpoints Saved | ' +
    report.event_counts.checkpoints_saved +
    ' |\n' +
    '| Artifacts Created | ' +
    report.event_counts.artifacts_created +
    ' |\n' +
    '| Tool Simulations | ' +
    report.event_counts.tool_simulations +
    ' |\n\n' +
  ...
  console.log(
    'Peak Memory:',
    report.memory_metrics.heap_peak_mb + 'MB (' + report.memory_metrics.peak_usage_percent + '%)'
  );
  ...
  if (fs.existsSync(testDir)) {
    fs.readdirSync(testDir)
      .filter(f => f.startsWith('stress-artifact-'))
      .forEach(f => fs.unlinkSync(path.join(testDir, f)));
  }
