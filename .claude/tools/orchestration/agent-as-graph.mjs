...
export async function buildKnowledgeGraph() {
  ...
  for (const [agentName, agentConfig] of Object.entries(config.agent_routing || {})) {
    try {
      ...
      if (!description) {
        try {
          ...
          console.warn(`[Graph] Failed to load description for agent ${agentName}:`, error.message);
          description = '';
        }
      }
      ...
      for (const toolName of allowedTools) {
        try {
          ...
          let toolNode = graph.nodes.find(n => n.type === NodeType.TOOL && n.name === normalizedToolName);
          ...
          if (!toolNode) {
            ...
            try {
              ...
              console.warn(`[Graph] Tool ${normalizedToolName} not found in registry:`, error.message);
            }
          ...
          console.error(`[Graph] Error processing tool ${toolName} for agent ${agentName}:`, error.message);
    ...
    await writeFile(GRAPH_FILE, JSON.stringify(graph, null, 2), 'utf8');
    console.log(`[Graph] Knowledge graph built successfully: ${graph.nodes.length} nodes, ${graph.edges.length} edges`);
    return graph;
  } catch (error) {
...
async function getMCPTool(toolName, config) {
  try {
    ...
    const mcpTool = config.mcp_registry?.active_tools?.find(t => 
      normalizeToolName(t.name) === normalizedName || t.name === toolName
    );
    return mcpTool || null;
  } catch (error) {
...
export async function agentAsGraphRetrieve(query, topK = 5) {
  ...
  const graph = await loadGraph();
  const toolNodeIds = reranked
    .filter(r => r.type === NodeType.TOOL)
    .map(r => r.node.id);
  ...
  for (const agentNode of parentAgents) {
    if (!agentNodeIds.has(agentNode.id)) {
      ...
      const relatedTools = reranked.filter(r => {
        return r.type === NodeType.TOOL && graph.edges.some(e => 
          e.source === agentNode.id && 
          e.target === r.node.id
        );
      });
  ...
  finalAgents.sort((a, b) => b.score - a.score);
    ...
    console.log(`[Graph] Retrieved ${result.agents.length} agents (${result.retrieval_stats.agent_matches} direct, ${result.retrieval_stats.parent_agents_found} from tools)`);
    return result;
  } catch (error) {
