...
import { readArtifactRegistry, getRunDirectoryStructure, getArtifactsArray } from './run-manager.mjs';
...
export function createContextPacket(params) {
  ...
  const resolvedReferences = runId 
    ? resolveReferences(references, runId)
    : references;
    ...
    if (artifact) {
      ...
      if (existsSync(resolvedRef.path)) {
        try {
          ...
          resolvedRef.contentPreview = preview.length < JSON.stringify(content).length 
            ? preview + '...' 
            : preview;
        } catch (error) {
      ...
      if (existsSync(filePath)) {
        ...
        if (filePath.endsWith('.md') || filePath.endsWith('.txt') || filePath.endsWith('.json')) {
          try {
            ...
            resolvedRef.contentPreview = content.substring(0, 500) + 
              (content.length > 500 ? '...' : '');
          } catch (error) {
            // Can't read
          }
...
export function extractConstraintsFromArchitecture(architecturePath) {
  ...
  try {
    ...
    console.warn(`Warning: Could not extract constraints from ${architecturePath}: ${error.message}`);
    return [];
  }
}
