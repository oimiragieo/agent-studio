...
async function main() {
  ...
  if (command === 'check') {
    ...
    if (runIdIndex === -1 || runIdIndex === args.length - 1) {
      console.error('Usage: node document-gate.mjs check --run-id <id> [--document <name> | --task-type <type>]');
      process.exit(1);
    }
    ...
    if (runIdIndex === -1 || docIndex === -1 || approvedByIndex === -1) {
      console.error('Usage: node document-gate.mjs approve --run-id <id> --document <name> --approved-by <agent>');
      process.exit(1);
    }
