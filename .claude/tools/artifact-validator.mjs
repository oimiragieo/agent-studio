...
export async function validateArtifactContract(runId, step, options = {}) {
  const { requireSchema = true, requireGate = true, requireReasoning = true } = options;
  ...
  if (requireSchema && stepArtifacts.length > 0) {
    ...
    const artifactsWithoutSchema = stepArtifacts.filter(
      a => !a.schema || a.validationStatus === 'unknown'
    );

    if (artifactsWithoutSchema.length > 0) {
      warnings.push(
        `Some artifacts do not have schema validation: ${artifactsWithoutSchema.map(a => a.name).join(', ')}`
      );
    }
  }
...
export async function enforceArtifactContract(runId, step, options = {}) {
  ...
  if (!result.valid) {
    const errorMessage =
      `Artifact contract violation for step ${step}:\n` +
