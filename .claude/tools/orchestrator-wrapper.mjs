...
import { PHOENIX_SIGNALS, triggerPhoenixReset, resumeFromPhoenix, shouldTriggerPhoenixReset } from './phoenix-manager.mjs';
...
export async function runOrchestratorWrapper(runId, userRequest) {
  ...
  while (iteration < maxIterations) {
    ...
    try {
      ...
      console.log(`[Wrapper] Resuming from: Phase ${projectDb.current_phase || 'N/A'}, Step ${projectDb.current_step}`);
    } catch (error) {
    ...
    return new Promise((resolve, reject) => {
      orchestratorProcess.on('exit', async (exitCode) => {
        if (exitCode === EXIT_CODE_CONTEXT_LIMIT || exitCode === EXIT_CODE_RESET_WITH_PROMPT) {
          ...
          const nextRequest = exitCode === EXIT_CODE_RESET_WITH_PROMPT 
            ? resumeInfo.resurrectionPrompt 
            : userRequest;
