...
export class VectorStore {
  ...
  async addVector(id, vector, metadata = {}) {
    ...
    try {
      ...
      if (vectorArray.length !== this.options.dimension) {
        throw new Error(
          `Vector dimension mismatch: expected ${this.options.dimension}, got ${vectorArray.length}`
        );
      }
  ...
  async searchSimilar(queryVector, k = 10) {
    ...
    try {
      ...
      if (vectorArray.length !== this.options.dimension) {
        throw new Error(
          `Query vector dimension mismatch: expected ${this.options.dimension}, got ${vectorArray.length}`
        );
      }
...
export function createVectorStore(options = {}) {
  const defaultIndexPath = join(process.cwd(), '.claude', 'context', 'memory', 'vectors.hnsw');
