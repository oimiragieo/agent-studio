...
class PatternLearnerTestSuite {
  ...
  async testInitialization() {
    ...
    this.assert(this.learner !== null, 'Pattern learner instance created');
    ...
    this.assert(this.learner.CONFIDENCE_THRESHOLDS.MIN === 0.1, 'Confidence thresholds defined');

    this.assert(this.learner.FREQUENCY_THRESHOLDS.RARE === 1, 'Frequency thresholds defined');
  }
  ...
  async testRecordNewPattern() {
    ...
    this.assert(result.isNew === true, 'Pattern marked as new');

    this.assert(typeof result.id === 'number' && result.id > 0, 'Pattern ID assigned');

    this.assert(result.confidence === 0.1, 'Initial confidence is 0.1');

    this.assert(result.occurrenceCount === 1, 'Initial occurrence count is 1');
  ...
  async testRecordDuplicatePattern() {
    ...
    this.assert(secondResult.isNew === false, 'Pattern not marked as new on second recording');

    this.assert(secondResult.id === firstResult.id, 'Same pattern ID used');

    this.assert(secondResult.occurrenceCount === 2, 'Occurrence count incremented');

    this.assert(secondResult.confidence > firstResult.confidence, 'Confidence increased');
  }
  ...
  async testGetPattern() {
    ...
    this.assert(pattern !== null, 'Pattern retrieved successfully');

    this.assert(pattern.type === 'tool_sequence', 'Pattern type matches');
    ...
    this.assert(pattern.frequency === 1, 'Frequency matches');

    this.assert(typeof pattern.confidence === 'number', 'Confidence is a number');
  }
  ...
  async testGetFrequentPatterns() {
    ...
    this.assert(patterns.length <= 3, 'Limit respected');
  ...
  async testIncrementFrequency() {
    ...
    this.assert(updateResult.frequency === 6, 'Frequency incremented correctly (1 + 5 = 6)');
    ...
    // Verify in database
    const pattern = await this.learner.getPattern(
      'error_pattern',
      'error:TypeError:add type checking'
    );

    this.assert(pattern.frequency === 6, 'Database reflects updated frequency');
  }
  ...
  async testGetHighConfidencePatterns() {
    ...
    this.assert(highConfPatterns.length > 0, 'High-confidence patterns found');
  ...
  async testGetPatternsForAgent() {
    ...
    this.assert(devWorkflows.length > 0, 'Developer patterns found');
  ...
  async testCleanupLowConfidencePatterns() {
    ...
    this.assert(cleanupResult.deleted >= 1, 'Stale low-confidence pattern deleted');

    // Verify pattern is gone
    const pattern = await this.learner.getPattern('workflow', 'workflow:test');

    this.assert(pattern === null, 'Pattern no longer exists after cleanup');
  }
  ...
  async testGetStatistics() {
    ...
    this.assert(typeof stats.total === 'number' && stats.total > 0, 'Total count available');

    this.assert(typeof stats.byType === 'object', 'Statistics grouped by type');

    this.assert(Object.keys(stats.byType).length > 0, 'Statistics contain pattern types');
  ...
  async testPerformanceRecordPattern() {
    ...
    this.assert(duration < 5, `Record pattern completes in <5ms (actual: ${duration}ms)`);
  }
  ...
  async testPerformanceGetFrequentPatterns() {
    ...
    this.assert(duration < 10, `Get frequent patterns completes in <10ms (actual: ${duration}ms)`);
  }
  ...
  async testErrorHandlingInvalidPatternType() {
    ...
    this.assert(errorCaught, 'Error thrown for invalid pattern type');
  }
  ...
  async testErrorHandlingInvalidPatternData() {
    ...
    this.assert(errorCaught, 'Error thrown for invalid pattern data');
  }
  ...
  async testConfidenceCalculation() {
    ...
    this.assert(confidences.length > 0, 'Confidence values tracked');
    ...
    this.assert(isMonotonic, 'Confidence increases monotonically');

    this.assert(confidences[confidences.length - 1] < 1.0, 'Confidence capped below 1.0');
