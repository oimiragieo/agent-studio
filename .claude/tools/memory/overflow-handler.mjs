...
export class ContextOverflowHandler {
  ...
  constructor(database) {
    ...
    this.THRESHOLDS = {
      ...
      COMPRESS: 0.9, // 90% - compress old messages
  ...
  async detectOverflow(sessionId, currentTokens, maxTokens) {
    ...
    // Log warning
    console.warn(
      `[Overflow] Context usage at ${(usage * 100).toFixed(1)}% ` + `(threshold: ${threshold})`
    );
  ...
  async compressOldMessages(sessionId) {
    ...
    try {
      ...
      for (const msg of oldMessages) {
        ...
        // Update database
        updateStmt.run(compressed, compressedTokens, msg.content, msg.id);
      }
  ...
  async summarizeConversations(sessionId) {
    ...
    try {
      ...
      const oldConversations = this.db
        .prepare(
          `
  ...
  _generateSimpleSummary(firstMsg, lastMsg, title) {
    ...
    const userRequest =
      firstMsg.role === 'user' ? firstMsg.content.substring(0, 80) : 'User initiated conversation';
  ...
  async initiateHandoff(sessionId) {
    ...
    try {
      ...
      const criticalMessages = this.db
        .prepare(
          `
