...
export class MemoryInjectionManager {
    ...
    async initialize() {
        ...
        if (this.options.semanticSearchEnabled && !this.semanticMemory) {
            try {
                ...
                console.warn('[Memory Injection] Semantic search initialization failed, using FTS5 only:', error.message);
    ...
    async injectRelevantMemory(context) {
        ...
        try {
            ...
            const tokenBudget = contextTokenBudget || this.calculateTokenBudget(
            ...
            const relevantMemory = await this.calculateRelevantMemory(
                sessionId,
                toolName,
                toolParams
            );
    ...
    async calculateRelevantMemory(sessionId, toolName, params) {
        ...
        try {
            ...
            snippets.push(...recentMessages.map(msg => ({
                type: 'recent_message',
                content: msg.content,
                role: msg.role,
    ...
    formatMemoryForInjection(rankedMemory, tokenBudget) {
        ...
        const memory = memoryParts.length > 0
    ...
    async calculateRelevantMemoryWithSemantics(sessionId, toolName, params) {
        ...
        try {
            ...
            console.log(`[Memory Injection] Combined search completed in ${duration}ms (FTS5: ${ftsResults.length}, Semantic: ${semanticResults.length})`);

            return mergedResults;

        } catch (error) {
            console.warn('[Memory Injection] Semantic search failed, falling back to FTS5:', error.message);
            return await this.calculateRelevantMemory(sessionId, toolName, params);
        }
    }
