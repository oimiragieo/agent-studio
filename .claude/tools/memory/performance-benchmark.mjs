...
class PerformanceBenchmark {
  ...
  async seedTestData() {
    ...
    const conversationId = this.db.createConversation({
      sessionId,
      conversationId: 'bench-conv-' + randomUUID(),
      title: 'Benchmark',
    });
  ...
  async scenario1_SingleAgentHandoff(testData) {
    ...
    await this.warmup(async () => {
      await this.handoffService.prepareHandoff({
        sessionId,
    ...
    for (let i = 0; i < BENCHMARK_CONFIG.iterations; i++) {
      ...
      await this.handoffService.prepareHandoff({
        sessionId,
        sourceAgentId: 'agent-' + (i % 10),
        targetAgentId: 'agent-' + ((i + 1) % 10),
        targetTask: 'Task ' + i,
        maxMemories: 10,
      });
    ...
    this.results.scenarios.singleAgentHandoff = {
      name: 'Single Agent Handoff',
      target: '<' + PERFORMANCE_TARGETS.handoff_p95 + 'ms p95',
      ...stats,
      meetsTarget,
      rawTimings: timings,
    };
    ...
    console.log(
      '  Target: ' +
        ...
        stats.p95.toFixed(2) +
        'ms vs <' +
        PERFORMANCE_TARGETS.handoff_p95 +
        'ms)'
    );
    return meetsTarget;
  }
  ...
  async scenario2_MultiAgentChain(testData) {
    ...
    this.results.scenarios.multiAgentChain = {
      name: 'Multi-Agent Chain',
      target: '<' + chainTarget + 'ms p95',
      handoffsPerChain: 3,
      ...stats,
      meetsTarget,
      rawTimings: timings,
  ...
  async scenario3_SessionResume(testData) {
    ...
    const timings = [],
      timer = new Timer(),
      { sessionId } = testData;
    const checkpoint = await this.resumeService.createCheckpoint({
      sessionId,
      checkpointType: 'manual',
      agentsInvolved: ['analyst', 'developer', 'qa'],
    });
    await this.warmup(async () => {
      await this.resumeService.resumeSession({
    ...
    this.results.scenarios.sessionResume = {
      name: 'Session Resume',
      target: '<' + PERFORMANCE_TARGETS.resume_p95 + 'ms p95',
      memoriesResumed: checkpoint.memoryCount,
      entitiesResumed: checkpoint.entityCount,
      ...stats,
      meetsTarget,
      rawTimings: timings,
    };
    ...
    console.log(
      '  Target: ' +
        ...
        stats.p95.toFixed(2) +
        'ms vs <' +
        PERFORMANCE_TARGETS.resume_p95 +
        'ms)'
    );
    return meetsTarget;
  }
  ...
  async scenario4_EntityDeduplication() {
    ...
    for (let i = 0; i < BENCHMARK_CONFIG.entityCount; i++) {
      entityValues.push({
        type: entityTypes[i % 5],
        value: 'Entity-' + entityTypes[i % 5] + '-' + (i % 200),
      });
    }
    for (const { type, value } of entityValues) {
      timer.begin();
      await this.entityRegistry.getGlobalEntity({
        type,
        value,
    ...
    this.results.scenarios.entityDeduplication = {
      name: 'Entity Deduplication',
      target: '<' + PERFORMANCE_TARGETS.entity_lookup_p95 + 'ms p95',
      entitiesProcessed: BENCHMARK_CONFIG.entityCount,
      uniqueEntities: entityStats.globalEntities.reduce((s, e) => s + e.count, 0),
      totalMerges,
      ...stats,
      meetsTarget,
      rawTimings: timings,
    ...
    console.log(
      '  Target: ' +
        ...
        stats.p95.toFixed(2) +
        'ms vs <' +
        PERFORMANCE_TARGETS.entity_lookup_p95 +
        'ms)'
    );
    return meetsTarget;
  }
  ...
  async scenario5_CacheEffectiveness(testData) {
    ...
    const timings = [],
      timer = new Timer(),
      { sessionId, conversationId } = testData;
    ...
    for (const query of queries) {
      await this.contextInjector.injectEnhancedMemory({
        sessionId,
        conversationId,
        query,
        tokenBudget: 5000,
    ...
    this.results.scenarios.cacheEffectiveness = {
      name: 'Cache Effectiveness',
      target: '>=' + PERFORMANCE_TARGETS.cache_hit_rate + '% hit rate',
      queriesExecuted: BENCHMARK_CONFIG.queryCount,
  ...
  async scenario6_CircularDetection(testData) {
    ...
    for (let i = 0; i < agents.length - 1; i++) {
      await this.collaborationManager.registerCollaboration({
        sessionId,
    ...
    for (let i = 0; i < BENCHMARK_CONFIG.iterations; i++) {
      timer.begin();
      await this.collaborationManager.detectCircularHandoff(
        sessionId,
        agents[i % 5],
        agents[(i + 2) % 5]
      );
    ...
    this.results.scenarios.circularDetection = {
      name: 'Circular Handoff Detection',
      target: '<' + PERFORMANCE_TARGETS.circular_detection_p95 + 'ms p95',
      agentsInGraph: agents.length,
      ...stats,
      meetsTarget,
      rawTimings: timings,
    };
    ...
    console.log(
      '  Target: ' +
        ...
        stats.p95.toFixed(2) +
        'ms vs <' +
        PERFORMANCE_TARGETS.circular_detection_p95 +
        'ms)'
    );
    return meetsTarget;
  }
  ...
  async runAll() {
    ...
    try {
      ...
      console.log(
        '  Overall Result: ' + (scenariosPassed >= 5 ? 'PASS' : 'FAIL') + ' (95% threshold: 5/6)'
      );
      return this.results;
    } finally {
  ...
  saveResults() {
    ...
    for (const scenario of Object.values(jsonResults.scenarios)) {
      if (scenario.rawTimings) {
        scenario.rawTimingsCount = scenario.rawTimings.length;
        delete scenario.rawTimings;
      }
    }
    writeFileSync(
      join(reportsDir, 'phase-5-benchmark-data.json'),
  ...
  generateMarkdownReport(reportsDir) {
    const s = this.results.scenarios,
      m = this.results.metadata,
      sum = this.results.summary;
    const report =
      '# Phase 5 Wave 2: Performance Benchmark Report\n\n## Executive Summary\n\n| Metric | Result | Target | Status |\n|--------|--------|--------|--------|\n| Scenarios Completed | ' +
      sum.scenariosCompleted +
      '/6 | 6 | ' +
      (sum.scenariosCompleted === 6 ? 'PASS' : 'FAIL') +
      ' |\n| Scenarios Meeting Targets | ' +
      sum.scenariosMeetingTargets +
      '/6 | >=5 | ' +
      (sum.scenariosMeetingTargets >= 5 ? 'PASS' : 'FAIL') +
      ' |\n| Total Execution Time | ' +
      (sum.totalExecutionTimeMs / 1000).toFixed(2) +
      's | <5min | ' +
      (sum.totalExecutionTimeMs < 300000 ? 'PASS' : 'FAIL') +
      ' |\n\n## Test Environment\n\n| Property | Value |\n|----------|-------|\n| Timestamp | ' +
      m.timestamp +
      ' |\n| Node.js Version | ' +
      m.nodeVersion +
      ...
      (s.singleAgentHandoff?.p50?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.singleAgentHandoff?.p95?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.singleAgentHandoff?.p99?.toFixed(2) || 'N/A') +
      'ms | <200ms | ' +
      (s.singleAgentHandoff?.meetsTarget ? 'PASS' : 'FAIL') +
      ' |\n| Multi-Agent Chain | ' +
      (s.multiAgentChain?.p50?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.multiAgentChain?.p95?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.multiAgentChain?.p99?.toFixed(2) || 'N/A') +
      'ms | <600ms | ' +
      (s.multiAgentChain?.meetsTarget ? 'PASS' : 'FAIL') +
      ' |\n| Session Resume | ' +
      (s.sessionResume?.p50?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.sessionResume?.p95?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.sessionResume?.p99?.toFixed(2) || 'N/A') +
      'ms | <1000ms | ' +
      (s.sessionResume?.meetsTarget ? 'PASS' : 'FAIL') +
      ' |\n| Entity Deduplication | ' +
      (s.entityDeduplication?.p50?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.entityDeduplication?.p95?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.entityDeduplication?.p99?.toFixed(2) || 'N/A') +
      'ms | <10ms | ' +
      (s.entityDeduplication?.meetsTarget ? 'PASS' : 'FAIL') +
      ' |\n| Cache Effectiveness | ' +
      (s.cacheEffectiveness?.p50?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.cacheEffectiveness?.p95?.toFixed(2) || 'N/A') +
      'ms | - | ' +
      (s.cacheEffectiveness?.cacheHitRate || 'N/A') +
      '% | ' +
      (s.cacheEffectiveness?.meetsTarget ? 'PASS' : 'FAIL') +
      ' |\n| Circular Detection | ' +
      (s.circularDetection?.p50?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.circularDetection?.p95?.toFixed(2) || 'N/A') +
      'ms | ' +
      (s.circularDetection?.p99?.toFixed(2) || 'N/A') +
      'ms | <100ms | ' +
      (s.circularDetection?.meetsTarget ? 'PASS' : 'FAIL') +
