...
function populateTestData(db) {
    ...
    db.addMessage({
        conversationId,
        role: 'assistant',
        content: 'Memory injection must complete in <100ms, with token budget of 20% of remaining context.',
        tokenCount: 18
    });
...
async function runTests() {
    ...
    try {
        ...
        if (score2 < score1) {
            ...
            console.log(`  ❌ Old message score: ${score2.toFixed(2)} (should be < ${score1.toFixed(2)})`);
            results.failed++;
        }
    ...
    try {
        ...
        const scoresDescending = ranked.every((snippet, i) =>
            i === 0 || snippet.score <= ranked[i - 1].score
        );
    ...
    try {
        ...
        if (formatted.tokensUsed <= 60 && formatted.sources.length === 2) {
            console.log(`  ✅ Token budget respected: ${formatted.tokensUsed}/60 tokens, ${formatted.sources.length} snippets`);
            results.passed++;
        } else {
            console.log(`  ❌ Token budget violated: ${formatted.tokensUsed}/60 tokens, ${formatted.sources.length} snippets`);
            results.failed++;
        }
    ...
    try {
        ...
        if (result.tokensUsed !== undefined && result.sources !== undefined) {
            console.log(`  ✅ Result structure valid (tokens: ${result.tokensUsed}, sources: ${result.sources.length})`);
            results.passed++;
        } else {
