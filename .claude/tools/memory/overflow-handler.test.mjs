        ...
        prepare: (sql) => ({
            ...
            run: (...args) => {
                ...
                if (sql.includes('UPDATE messages SET content')) {
                    ...
                    if (msg) { msg.content = args[0]; msg.is_summarized = true; }
                }
                ...
                if (sql.includes('UPDATE messages SET is_summarized = TRUE')) {
                    mockData.messages.filter(m => m.conversation_id === args[0]).forEach(m => m.is_summarized = true);
                }
                if (sql.includes('INSERT INTO session_handoffs')) {
                    mockData.handoffs.push({ from_session_id: args[0], to_session_id: args[1], handoff_reason: args[4] });
                }
                return { changes: 1 };
            }
        })
...
function generateMockMessages(count, options = {}) {
    return Array.from({ length: count }, (_, i) => ({
        ...
        content: options.content || 'Test message ' + (i + 1) + ' with content for testing overflow handler that is long enough to exceed limit.',
...
async function testOverflowDetection() {
    ...
    } catch (e) { recordTest('Usage < 85% returns none', false, e.message); }
    ...
    } catch (e) { recordTest('Usage 85-90% returns warn', false, e.message); }
    ...
    } catch (e) { recordTest('Usage 90-93% returns compress', false, e.message); }
    ...
    } catch (e) { recordTest('Usage 93-97% returns summarize', false, e.message); }
    ...
    } catch (e) { recordTest('Usage 97%+ returns handoff', false, e.message); }
}
...
async function testCompression() {
    ...
    try {
        const messages = generateMockMessages(60, { 
            content: 'This is a very long message that should be compressed because it exceeds the 100 character limit for the overflow handler.',
            tokenCount: 100
        });
    ...
    } catch (e) { recordTest('compressOldMessages compresses correctly', false, e.message); }
    ...
    } catch (e) { recordTest('Compression preserves last 10 messages', false, e.message); }
}
...
async function testSummarization() {
    ...
    try {
        const conversations = [
            { id: 1, conversation_id: 'conv-1', session_id: 'sess-1', title: 'Test Conv 1', summary: null, ended_at: '2025-01-01' }
        ];
        const messages = [
    ...
    } catch (e) { recordTest('summarizeConversations creates summaries', false, e.message); }

    try {
        const conversations = [
            { id: 1, conversation_id: 'conv-1', session_id: 'sess-1', title: null, summary: null, ended_at: '2025-01-01' }
        ];
        const messages = [
    ...
    } catch (e) { recordTest('Conversations marked as summarized', false, e.message); }
}
...
async function testHandoff() {
    ...
    try {
        const sessions = [{ session_id: 'sess-1', user_id: 'user-1', project_id: 'proj-1', status: 'active' }];
        const conversations = [{ id: 1, conversation_id: 'conv-1', session_id: 'sess-1', title: 'Test', summary: 'Summary' }];
    ...
    } catch (e) { recordTest('initiateHandoff creates new session', false, e.message); }

    try {
        const sessions = [{ session_id: 'sess-old', user_id: 'user-1', project_id: 'proj-1', status: 'active' }];
        const conversations = [{ id: 1, conversation_id: 'conv-1', session_id: 'sess-old', title: 'Old', summary: null }];
    ...
    } catch (e) { recordTest('Old session archived and handoff recorded', false, e.message); }
}
...
async function testIntegration() {
    ...
    } catch (e) { recordTest('handleOverflow orchestrates correctly', false, e.message); }

    try {
        const sessions = [{ session_id: 'sess-prog', user_id: 'user-1', project_id: 'proj-1', status: 'active' }];
        const conversations = [{ id: 1, conversation_id: 'conv-1', session_id: 'sess-prog', title: 'Test', summary: null, ended_at: '2025-01-01' }];
        const messages = generateMockMessages(50, { tokenCount: 100 });

        let mockDb = createMockDatabase({ sessions: [...sessions], conversations: [...conversations], messages: [...messages] });
        let handler = new ContextOverflowHandler(mockDb);
        let result = await handler.handleOverflow('sess-prog', 90000, 100000);
        assert.strictEqual(result.action, 'compressed');

        mockDb = createMockDatabase({ sessions: [...sessions], conversations: [...conversations], messages: [...messages] });
    ...
    } catch (e) { recordTest('Progressive compaction (90%->93%->97%)', false, e.message); }
}
...
async function testTokenEstimation() {
    ...
    } catch (e) { recordTest('Token estimation calculates correctly', false, e.message); }
}
