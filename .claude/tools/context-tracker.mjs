...
function getContextLimit() {
  try {
    ...
    console.warn(
      `⚠️  Warning: Could not read max_context_tokens from settings.json: ${error.message}. Using default 100000.`
    );
    return 100000;
  }
}
...
export function shouldTriggerHandoff(runId, contextUsage) {
  ...
  const thresholds = {
    high: 0.9, // 90% for high confidence
    medium: 0.85, // 85% for medium confidence
    low: 0.8, // 80% for low confidence
  };
