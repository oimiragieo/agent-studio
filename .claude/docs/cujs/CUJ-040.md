...
**Workflow File**: `.claude/workflows/incident-response.yaml`
...
| -------------------------------- | ------------------------------- | ----------------------------------------------------------------------------------------- |
| System recovers from file system | Gate validation                 | Recovery using file system state only                                                     |
...
| No conversation references       | Reasoning file content analysis | No "previous conversation" references                                                     |
| Plan read from file system       | File read validation            | Plan read from file, not memory                                                           |
| Gate files checked               | Completion status validation    | Actual status determined from gate files                                                  |
| Plan updates reflect file system | Plan synchronization            | Updates based on actual file system state                                                 |
| Workflow continuity              | Gate validation                 | Seamless continuation after recovery                                                      |
| Stateless behavior validated     | Validation checkpoints          | All checkpoints pass                                                                      |
| File read timestamps logged      | Timestamp logging               | Logged in `reasoning-{step}.json`                                                         |
