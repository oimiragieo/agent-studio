...
**Note**: This CUJ executes via direct skill invocation without requiring agent spawning or workflow orchestration.
...
|-----------|-------------|--------|
| Tech stack correctly identified | Stack detection result or user input parsed | Technologies detected accurately or user description parsed if empty directory |
| Rule index loaded | Rule index file loaded or fallback directory scan completed | Pre-built index loaded or fallback to directory scan successful |
| Relevant rules selected | Rule selection query result | Rules matching tech stack selected |
| Irrelevant rules excluded | Rule exclusion filter result | Non-matching rules excluded |
| Configuration valid | manifest.yaml validation | Valid YAML with correct rule references |
| Works out-of-the-box | Installation test without index generation | Functions without requiring `pnpm index-rules` |
