...
**Workflow File**: `.claude/workflows/multi-platform-deployment.yaml`
...
- **Agent**: QA
- **Actions**:
  - Load `.claude/context/config/skill-integration-matrix.json`
...
- **Actions**:
  ...
  - Update `.claude/context/config/skill-integration-matrix.json`
...
- Skill inventory: `.claude/context/artifacts/generated/skill-inventory-{id}.json`
- Mapping validation: `.claude/context/artifacts/generated/skill-mapping-validation-{id}.json`
- Invocation results: `.claude/context/artifacts/generated/skill-invocation-results-{id}.json`
- Workflow integration: `.claude/context/artifacts/generated/workflow-skill-integration-{id}.json`
- Performance report: `.claude/context/artifacts/generated/skill-performance-{id}.json`
- Cross-platform report: `.claude/context/artifacts/generated/skill-cross-platform-{id}.json`

## Success Criteria

| Criterion                         | Measurement              | Target                                  |
| --------------------------------- | ------------------------ | --------------------------------------- |
| All skills discovered             | Skill inventory          | 43 skills inventoried                   |
| Agent skill mappings correct      | Mapping validation       | 34 agents with correct mappings         |
| Required skills accessible        | Accessibility validation | All required skills available to agents |
| Natural language invocation works | Invocation testing       | Works for all 43 skills                 |
| Skill tool invocation works       | Tool invocation testing  | Works for all 43 skills                 |
| Skills trigger correctly          | Workflow trigger testing | Triggers validated in workflows         |
| Skill outputs match schemas       | Schema validation        | Outputs conform to schemas              |
| No critical integration issues    | Issue analysis           | No blocking issues                      |
| Performance within limits         | Performance benchmarks   | Execution time within targets           |
| Cross-platform compatibility      | Platform testing         | Validated on Claude, Cursor, Factory    |
...
```
For each agent in `.claude/context/config/skill-integration-matrix.json`:
...
```
For each skill trigger in `.claude/context/config/skill-integration-matrix.json`:
...
- [skill-integration-matrix.json](../../context/config/skill-integration-matrix.json) - **Primary reference for agent-skill mappings**
...
- [ ] `.claude/context/config/skill-integration-matrix.json` loaded
...
**Resolution**: Update trigger conditions in `.claude/context/config/skill-integration-matrix.json`
...
- **Alternative Agent**: If primary agent fails 3x, route to backup agent
  ...
  - Cursor-Validator → QA (cross-platform validation fallback)
  - Performance-Engineer → Developer (performance testing fallback)
- **Manual Override**: User can force-proceed with documented risks
- **Graceful Degradation**: Continue with partial skill validation (skip problematic skills)
...
- Skill state: `.claude/context/runtime/runs/{{run_id}}/skill-validation-state.json`
