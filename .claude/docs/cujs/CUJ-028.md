...
- Analyzes cloud infrastructure requirements
...
| --------------------------------------------- | ----------------------------------------------- | -------------------------------------------------------------------- |
| Resource names defined before coding          | infrastructure-config.json exists before Step 7 | File created at Step 4.5 with all resource names                     |
| No rework needed when resources provisioned   | Code modification count post-provisioning       | Zero code changes required after Step 11                             |
| Connection strings use actual resource names  | Code analysis for placeholder usage             | All connection strings use resources from infrastructure-config.json |
| Secrets properly referenced (never hardcoded) | Security scan for hardcoded secrets             | Zero hardcoded secrets, all use Secret Manager references            |
| Infrastructure provisioned successfully       | Terraform apply exit code                       | Exit code 0, all resources created                                   |
| Security validation passes                    | security-architect gate validation              | Gate file shows `allowed: true` for Step 4.5                         |
...
- Infrastructure config: `.claude/context/runtime/runs/{{run_id}}/infrastructure-config.json`
