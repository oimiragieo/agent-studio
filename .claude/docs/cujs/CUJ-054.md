...
- [ ] Publishing metadata synced to Cursor platform (artifact: `.cursor/plans/artifacts/<artifact-id>.json`, validated by schema)
- [ ] Metadata structure consistent across platforms (validated by `.claude/schemas/artifact-metadata.schema.json`)
- [ ] `last_synced` timestamp updated on sync (validated by timestamp comparison)
- [ ] Conflict resolution works: most recent `published_at` preferred (validated by conflict resolution log)
- [ ] Bidirectional sync works: Cursor → Claude and Claude → Cursor (validated by sync report)
- [ ] Schema validation catches invalid metadata (validated by error logs)
- [ ] All publishing fields synced: `published`, `published_at`, `publish_status`, `publish_targets`, `publish_attempts` (validated by field comparison)
