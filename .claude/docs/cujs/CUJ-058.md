...
- Verify checkpoint was saved to `.claude/context/runtime/runs/{{run_id}}/checkpoints`
- Resume workflow from checkpoint
- Confirm workflow continues from last successful step

### Step 5: Error Recovery Validation

- Review recovery logs in `.claude/context/runtime/runs/{{run_id}}/recovery.json`
...
- `.claude/context/runtime/runs/{{run_id}}/checkpoints/step-{{n}}-checkpoint.json`: Checkpoint files for each step
- `.claude/context/runtime/runs/{{run_id}}/recovery.json`: Recovery log with retry attempts and fallback activations
- `.claude/context/runtime/runs/{{run_id}}/workflow-state.json`: Updated workflow state after recovery

## Success Criteria

| Criterion                                                | Measurement                       | Target                                                                     |
| -------------------------------------------------------- | --------------------------------- | -------------------------------------------------------------------------- |
| Workflow detects retryable errors correctly              | Error classification validation   | Timeout, network_error, rate_limit classified as retryable                 |
| Retry logic respects max_attempts and backoff_strategy   | Retry count and timing validation | Max 3 attempts with exponential backoff (1s, 2s, 4s)                       |
| Fallback agents activate when primary agent fails        | Fallback agent execution logs     | Fallback agent invoked after primary exhausts retries                      |
| Checkpoints created at configured intervals              | Checkpoint file existence         | Checkpoint files in `.claude/context/runtime/runs/{{run_id}}/checkpoints/` |
| Workflow resumes from checkpoint after interruption      | Resume step validation            | Workflow continues from last successful step in checkpoint                 |
| Recovery logs complete and actionable                    | Recovery log completeness         | All retries and fallbacks logged in `recovery.json`                        |
| Non-retryable errors fail gracefully with clear messages | Error message validation          | Validation errors fail immediately with actionable messages                |
