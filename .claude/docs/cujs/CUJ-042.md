...
| ------------------------------------- | ------------------------------- | ----------------------------------------------------------------------------------------- |
| Cursor subagents activated            | Coordination log                | All subagents activated successfully                                                      |
...
| Native Cursor tools used              | Coordination log                | Cursor subagent tools used for delegation                                                 |
| Artifacts shared via Cursor directory | Artifact location validation    | Artifacts in `.cursor/plans/artifacts/*.json`                                             |
| Context passed correctly              | Context file validation         | Context in `.cursor/plans/current-context.json`                                           |
| Cursor Plan Mode integrated           | Plan Mode artifact              | Implementation plan in `.cursor/plans/plan-{id}.json`                                     |
| Plan Mode artifacts linked            | Plan linking validation         | Links between Plan Mode and Planner Agent plans                                           |
| No coordination failures              | Gate validation                 | Coordination succeeds without failures                                                    |
| Seamless handoffs                     | Handoff log                     | Handoffs documented in `handoff-log.json`                                                 |
...
2. Plan saved to `.claude/context/artifacts/generated/plan-{{id}}.json`
