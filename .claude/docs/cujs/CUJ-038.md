...
| --------------------------------------------- | ------------------------------- | ----------------------------------------------------------------------------------------- |
| Workflow continues without optional artifacts | Gate validation                 | Workflow succeeds despite missing optional artifacts                                      |
...
| Defaults used appropriately                   | Reasoning file validation       | Defaults applied when optional artifacts missing                                          |
| Optional input unavailability documented      | Reasoning files                 | Documented in `reasoning-{step}.json`                                                     |
| Workflow resilience                           | Gate validation                 | No failures due to missing optional inputs                                                |
| Optional artifact checks                      | Reasoning file validation       | Agents verify optional artifacts before use                                               |
| Multiple optional artifacts handled           | Workflow completion validation  | All optional artifacts handled correctly                                                  |
| Workflow completion                           | Gate validation                 | Workflow succeeds in all scenarios                                                        |
