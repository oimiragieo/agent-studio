...
- Analyzes cloud service integration requirements
...
| ---------------------------------------- | ------------------------------- | -------------------------------------------------------- |
| Business logic separated from cloud code | Code structure analysis         | Cloud code in separate modules from business logic       |
| Cloud integration works with emulators   | Emulator connectivity test      | All cloud services accessible via local emulators        |
| Production-ready cloud clients           | Cloud client validation         | Clients work with both emulator and production endpoints |
| Proper error handling                    | Error handling pattern analysis | All cloud calls include retry logic and error handling   |
| Authentication configured correctly      | Auth config validation          | ADC or service account credentials properly configured   |
| Emulator validation passed               | Emulator integration test       | All emulator tests pass with exit code 0                 |
...
- Cloud integration state: `.claude/context/runtime/runs/{{run_id}}/cloud-integration.json`
