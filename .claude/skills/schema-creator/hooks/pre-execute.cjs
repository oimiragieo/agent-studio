...
if (!stateCreated) {
  console.warn(`[${CREATOR_NAME.toUpperCase()}] Warning: Could not create state file. unified-creator-guard may block schema writes.`);
}
...
console.log(`[${CREATOR_NAME.toUpperCase()}] Pre-execute complete. Schema-creator workflow is now active.`);
process.exit(0);
