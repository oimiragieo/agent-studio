"""
Executor for git Skill - Handles dynamic MCP tool calls.

This executor connects to the git MCP server and executes tool calls
...
class GitExecutor:
    """Executor for git MCP server tools."""

    def __init__(self, config_path: Optional[Path] = None, repo_path: Optional[Path] = None):
        ...
        args = self.config.get("args", []) + ["--repository", str(repo_path)]
...
async def main():
    ...
    parser = argparse.ArgumentParser(description=f"Execute git MCP tools")
